...
with TemporaryDirectory() as directory:
    for mode in ('current', 'edit', 'save_as', 'payload', 'project', 'cancel'):
        ...
        with patch('core.settings.load_settings', return_value=SimpleNamespace(cache_dir=folder)), patch('core.transcription.transcribe_video', return_value=[]):
            ...
            assert worker.job_status == 'completed', worker.result
            receipt = worker.cache.results[audio.id]
            ...
            if mode == 'current':
                ...
                if mode != 'edit': assert audio.transcript is None
