...
- The MCP server splits long-running ops (scene detection, analysis, downloads) into `start_*` / `get_job_status` / `get_job_result` / `cancel_job` tools backed by `scene_ripper_mcp/jobs/` (SQLite store, per-project mutex, `ThreadPoolExecutor` runtime). The framework is MCP-only; the GUI keeps using `ui/workers/CancellableWorker`. Caller-facing reference: `docs/user-guide/headless-mcp.md`.
