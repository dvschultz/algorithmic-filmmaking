...
- `core/spine/` is the GUI-agnostic shared layer below `core/chat_tools.py` (the GUI agent's tool registry) and `scene_ripper_mcp/tools/*` (the MCP server). Both surfaces import from spine; neither imports the other. Spine modules MUST NOT pull `PySide6`, `mpv`, `av`, `faster_whisper`, `paddleocr`, or `mlx_vlm` into `sys.modules` at module load — `tests/test_spine_imports.py` is the boundary test that enforces this. Heavy or GUI-bound deps go inside function bodies (lazy import). New project-only tool implementations belong in `core/spine/<topic>.py`; the chat-tools and MCP wrappers are thin delegations.
- The MCP server splits long-running ops (scene detection, analysis, downloads) into `start_*` / `get_job_status` / `get_job_result` / `cancel_job` tools backed by the GUI-agnostic `core/jobs/` runtime (SQLite store, per-project mutex, `ThreadPoolExecutor`). `scene_ripper_mcp/jobs/` retains compatibility exports. New desktop lifecycle integrations use `ui/workers/job_adapter.py`; existing `CancellableWorker` workflows remain until migrated. Core jobs must not import MCP or Qt. Caller-facing reference: `docs/user-guide/headless-mcp.md`.
...
- Preserve `main.py` startup ordering. Frozen environment setup must happen before heavy imports, `torch` should remain pre-imported before `PySide6` while any runtime family still runs it in-process (`core.runtime_families.host_needs_runtime`), and macOS MLX warmup must stay on the main thread.
...
- `core/runtime_worker/` runs under the managed interpreter: keep its modules stdlib-only at import time (engine code is imported lazily inside the `analysis` task through the `core/runtime_worker/calls.py` allowlist). Adding a native feature means a `FEATURE_DEPS` entry, a profile in `core/runtime_profiles.py`, an `@isolated(family, call)` seam plus allowlist entry, and a `PROBE_MODULES` entry for the family's runtime.
