...
- Scope delivered: the transcription family (U13's proven worker). Embeddings, local VLMs, OCR, faces/objects/gaze, alignment and audio/stem runtimes keep the in-place installer until each family has packaged worker smoke evidence; the Torch/MLX GUI startup workarounds in `main.py` stay until no GUI path imports those runtimes (removal condition recorded in `docs/architecture/surface-compatibility.md`).
