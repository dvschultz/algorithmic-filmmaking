...
- Review closure (family migration): the CE review of `f465cc3..af05165` (run `20260909-073111-2583a8a3`, 9 reviewer artifacts) found host-side readiness gates still importing isolated runtimes, dataclass arguments silently dropped at the seam, `ModelDownloadError` degrading to a generic failure, results above the 1 MiB protocol bound, workers deriving source-mode config/cache/ffmpeg paths, missing per-call timeouts, and library stdout able to corrupt the protocol; the follow-up commit applies 22 fixes with regression tests (worker fd 1 rerouted to stderr, `worker_engine` feature installed with every profile, host hands config/cache/ffmpeg to workers, keyring unreadable in workers, `unload_model`/warm-up calls isolated, VLM provenance decided in the worker, `progress_cb`/cancel relayed, selftest calls gated). The `native-analysis` smoke was rerun locally with every family enabled through the public engine functions (all five ok) and the full suite passes (6316 tests). Deferred: worker-side package versions in provenance records; a per-family memory budget.
