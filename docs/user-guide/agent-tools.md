...
Runtime profile tools: `list_runtime_profiles`, `get_runtime_profile_status` (with `probe` for a worker health check), `install_runtime_profile` and `rollback_runtime_profile` manage the isolated native runtimes (`transcription-whisper` today) exactly like the desktop install prompt and the `scene_ripper runtime` CLI: staged install, worker health check, promotion, previous runtime kept on failure.
