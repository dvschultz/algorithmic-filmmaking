...
Every sequencer now runs through the shared algorithm registry and stores a recipe on the generated sequence. Recipe tools: `get_sequence_recipe` (inspect parameters, seed, inputs, realized placements, whether the timeline still matches), `reconstruct_sequence` (replay without any model call), `regenerate_sequence` (new variation with overrides), `duplicate_sequence`, `compare_sequences` (A/B: clip counts, durations, seeds, changed parameters, lineage, identical timelines; the same data the Sequence tab's Compare A/B panel shows), `list_sequences`, and `activate_sequence`. For `generate_remix`, `shuffle` and `color` specifically: the generated sequence stores a recipe (parameters, seed, inputs, realized order and transforms) and the tool result includes `recipe_id` and `seed`. For `shuffle`, `seed: 0` keeps its legacy meaning of "pick a random seed"; the drawn seed is recorded so the result can be repeated. `list_sorting_algorithms` shows the registry schema under `engine` for these algorithms.

Runtime profile tools: `list_runtime_profiles`, `get_runtime_profile_status` (with `probe` for a worker health check), `install_runtime_profile` and `rollback_runtime_profile` manage the isolated native runtimes (`transcription-whisper`, `vision-torch`, `ocr-paddle`, `vlm-local`, `audio-librosa`, `alignment-ctc`) exactly like the desktop install prompt and the `scene_ripper runtime` CLI: staged install where possible, worker health check, promotion, previous runtime kept on failure. `isolate_runtime_family("audio")` moves one family's inference into its isolated worker (`update_settings("native_worker_families", ...)` replaces the whole list).

`generate_remix` is the right tool for these 12 direct arrange/connect algorithms. Dialog-heavy sequencers with extra parameters use their own tools instead: `generate_reference_guided`, `generate_staccato`, `generate_rose_hobart`, `generate_eyes_without_a_face`, `generate_cassette_tape`, `generate_signature_style`, `generate_exquisite_corpus`, and `generate_storyteller`. Free Association, Word Sequencer, and LLM Word Composer have no dedicated chat tool; run them (and any other algorithm with its full registry parameters) through the generic `generate_sequence` chat tool, the MCP `generate_sequence`/`start_generate_sequence` tools, or `scene_ripper sequence generate`.
