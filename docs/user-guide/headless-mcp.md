...
- `compare_sequences(project_path, sequence_a, sequence_b)` is read-only: it
  ...
  both sides have a recipe, otherwise `null`), `inputs_equal`, `related`,
  `timelines_identical` (media, range, placement and transforms all equal)
  and the duration/clip deltas.
