...
@tools.register(
    description="Explicitly reuse legacy colors, brightness, volume, classification, object detection, gaze, shot types, or compatible DINO thumbnail/boundary embeddings for exact clip IDs. Only call when the user explicitly chooses legacy reuse. Provenance stays unknown; this does not verify how old values were computed. Returns after media checks and publication. Save the project to persist decisions.",
