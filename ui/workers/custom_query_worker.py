...
class CustomQueryWorker(CancellableWorker):
    ...
    Uses ThreadPoolExecutor for parallel processing. Cloud tier uses
    parallelism=3 (I/O-bound API calls), local tier uses parallelism=1
    (model not thread-safe).
    ...
    def run(self):
        ...
        with ThreadPoolExecutor(max_workers=self._parallelism) as executor:
            ...
            for future in as_completed(future_to_task):
                ...
                try:
                    clip_id, query, match, confidence, model, error_msg = future.result()
