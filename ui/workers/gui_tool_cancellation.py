...
def cancel_gui_tool_work(
    ...
    if window._chat_worker is None:
        return
    controllers = tuple(getattr(window, "_active_clip_analyses", ())) + tuple(
        getattr(window, "_active_source_imports", ())
    )
    ...
    workers.extend(getattr(window, "_active_legacy_reuses", ()))
    workers.extend(getattr(window, "_active_audio_imports", ()))
    workers.extend(getattr(window, "_active_audio_transcribes", ()))
