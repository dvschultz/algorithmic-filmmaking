...
class LegacyReuseWorker(CancellableWorker):
    ...
    def __init__(self, project: Project, operation: str, clip_ids: list[str], parent=None, *, settings: Settings | None = None) -> None:
        ...
        if self.request is not None:
            ...
            self._compute = partial(accept_legacy_alignment, alignment_tasks, cancel_event=self._cancel_event)
            ...
            self.application = CinematographyApplication(project, cinematography_tasks, cinematography_options)
            self._compute = partial(accept_legacy_cinematography, cinematography_tasks, cinematography_options, cancel_event=self._cancel_event)
        elif operation == "describe":
            description_tasks = tuple(description_task(project.clips_by_id[cid], project.sources_by_id.get(project.clips_by_id[cid].source_id)) for cid in ids)
            description_options = resolve_description_options(settings=settings)
            self.application = DescriptionApplication(project, description_tasks, description_options)
            self._compute = partial(accept_legacy_descriptions, description_tasks, description_options, cancel_event=self._cancel_event)
            ...
            self.application = ShotTypeApplication(project, shot_tasks, options)
            self._compute = partial(accept_legacy_shots, shot_tasks, options, cancel_event=self._cancel_event)
        elif operation == "gaze":
