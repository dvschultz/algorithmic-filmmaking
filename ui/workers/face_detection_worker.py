...
class FaceDetectionWorker(CancellableWorker):
    ...
    def _prepare(self) -> bool:
        if any(
            ...
            raise RuntimeError("Face detection media changed while queued")
    ...
    def run(self) -> None:
        ...
        def compute(progress, cancel):
            ...
            def report(current, total):
                progress(
                    ...
                    f"Face detection ({current}/{total})",
                )
        ...
        try:
            ...
            if row.status == "failed" and not self.result:
                raise RuntimeError(row.error or "Face detection failed")
        except Exception as exc:
            ...
            try:
                ...
                if errors and not self.is_cancelled():
                    ...
                    self.error.emit(
                        model_failure.message or "Face detection model unavailable"
                        if model_failure is not None
                        else summarize_clip_errors(
                            errors, operation_label="Face detection"
                        )
                    )
