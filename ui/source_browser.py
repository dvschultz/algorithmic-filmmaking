...
from ui.theme import theme, UISizes, TypeScale
...
class _SelectionContainer(QWidget):
    ...
    def __init__(self, browser: "SourceBrowser"):
        super().__init__()
...
class SourceBrowser(QWidget):
    ...
    def select_all(self) -> None:
        """Select all sources."""
        self._set_selected_ids({thumb.source.id for thumb in self.thumbnails})
    ...
    def _set_selected_ids(self, source_ids: set[str]) -> None:
        """Apply selection state and refresh thumbnail visuals."""
        self.selected_source_ids = set(source_ids)
        for thumb in self.thumbnails:
            thumb.set_selected(thumb.source.id in self.selected_source_ids)
    ...
    def _on_container_mouse_press(self, event) -> None:
        ...
        self._marquee_base_selection = set(self.selected_source_ids)
        event.accept()
    ...
    def _apply_marquee_selection(self, rect: QRect, additive: bool) -> None:
        """Apply replace or additive selection for sources intersecting the rect."""
        selected_ids = self._source_ids_in_rect(rect)
        if additive:
            selected_ids |= self._marquee_base_selection
        self._set_selected_ids(selected_ids)

    def _source_ids_in_rect(self, rect: QRect) -> set[str]:
        """Return source IDs whose cards intersect the marquee rect."""
        return {
            thumb.source.id
            for thumb in self.thumbnails
